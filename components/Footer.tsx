import { ArrowUpRight, Facebook, Instagram, MessageCircle, Music2, Phone } from "lucide-react";
import Link from "next/link";
import { Logo } from "@/components/Logo";
import { navItems, siteConfig } from "@/data/site";

function WhatsAppIcon() {
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12.05 0C5.5 0 .16 5.34.16 11.89c0 2.1.55 4.15 1.59 5.95L.06 24l6.34-1.66a11.87 11.87 0 0 0 5.65 1.44h.01c6.55 0 11.89-5.34 11.89-11.89A11.86 11.86 0 0 0 12.05 0Zm0 21.78h-.01a9.87 9.87 0 0 1-5.03-1.38l-.36-.21-3.74.98 1-3.65-.24-.37a9.87 9.87 0 0 1-1.51-5.26c0-5.45 4.43-9.88 9.88-9.88 2.64 0 5.12 1.03 6.99 2.9a9.83 9.83 0 0 1 2.89 6.99c0 5.45-4.43 9.88-9.87 9.88Zm5.42-7.4c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.16-.17.2-.35.22-.64.07-.3-.15-1.26-.46-2.39-1.48-.88-.79-1.48-1.76-1.65-2.06-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.03-.52-.07-.15-.67-1.61-.92-2.21-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.08-.79.37-.27.3-1.04 1.02-1.04 2.48s1.07 2.88 1.21 3.07c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.7.63.71.23 1.36.2 1.87.12.57-.09 1.76-.72 2.01-1.41.25-.69.25-1.29.17-1.41-.07-.12-.27-.2-.57-.35Z" /></svg>;
}

export function Footer() {
  return (
    <footer className="site-footer">
      <div className="site-footer__curve" />
      <div className="page-shell site-footer__cta">
        <div>
          <span className="eyebrow eyebrow--light">¿Tienes un proyecto especial?</span>
          <h2>Conversemos sobre el espacio que quieres transformar.</h2>
        </div>
        <Link className="button button--light" href="/cotizar">Diseñar mi proyecto <ArrowUpRight size={18} /></Link>
      </div>

      <div className="page-shell site-footer__grid">
        <div className="site-footer__brand">
          <Link href="/" aria-label="Industrial Remotos Perú"><Logo inverse /></Link>
          <p>Diseñamos, fabricamos e instalamos soluciones de acceso a medida para hogares, negocios e industrias.</p>
          <a className="site-footer__contact-line" href={`tel:+${siteConfig.whatsappNumber}`}><Phone size={16} />{siteConfig.phoneDisplay}</a>
        </div>

        <div className="site-footer__links">
          <h3>Navega</h3>
          {navItems.slice(1).map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}
        </div>

        <div className="site-footer__links">
          <h3>Legal</h3>
          <Link href="/politica-privacidad">Política de privacidad</Link>
          <Link href="/terminos">Términos y condiciones</Link>
        </div>

        <div className="site-footer__details">
          <h3>Canales vigentes</h3>
          <div><small>WhatsApp</small><b><MessageCircle size={14} /><a href={siteConfig.social.whatsapp} target="_blank" rel="noreferrer">Abrir conversación</a></b></div>
          <div><small>Solicitud web</small><b><Link href="/contacto">Ir a contacto</Link></b></div>
        </div>

        <div className="site-footer__follow">
          <h3>Síguenos</h3>
          <div className="socials">
            <a href={siteConfig.social.facebook} aria-label="Facebook" target="_blank" rel="noreferrer"><Facebook size={18} /></a>
            <a href={siteConfig.social.instagram} aria-label="Instagram" target="_blank" rel="noreferrer"><Instagram size={18} /></a>
            <a href={siteConfig.social.tiktok} aria-label="TikTok" target="_blank" rel="noreferrer"><Music2 size={18} /></a>
          </div>
          <p>Conoce nuestros proyectos, procesos y nuevas instalaciones.</p>
          <a className="site-footer__whatsapp" href={siteConfig.social.whatsapp} target="_blank" rel="noreferrer"><WhatsAppIcon />Habla con nosotros</a>
        </div>
      </div>

      <div className="page-shell site-footer__legal">
        <p>© 2026 Industrial Remotos Perú. Todos los derechos reservados.</p>
        <a href={siteConfig.developer.url} target="_blank" rel="noreferrer">Diseñado por {siteConfig.developer.name} <ArrowUpRight size={13} /></a>
      </div>
    </footer>
  );
}
